...
const title = computed(() => props.title || 'Gestión de Grupos');
const addButtonLabel = computed(() => props.addButtonLabel || 'Nuevo Grupo');
...
const exportToExcel = () => {
  ...
  const exportData = props.groups.map(group => ({
    ID: group.id,
    Código: group.codigo,
    Nombre: group.nombre
  }));
